...
fn bench_vrf_prove(c: &mut Criterion) {
    ...
    c.bench_function("vrf_prove", |b| {
        b.iter(|| keypair.prove(black_box(input)))
    });
}
...
fn bench_vrf_verify(c: &mut Criterion) {
    ...
    c.bench_function("vrf_verify", |b| {
        b.iter(|| verify_proof(black_box(keypair.public_key()), black_box(input), black_box(&proof)))
    });
}
...
criterion_group!(benches, bench_vrf_prove, bench_vrf_verify, bench_vrf_prove_and_verify);
