...
mod tests {
    ...
    fn test_overlay_rejects_transfer_with_utxo_mixing() {
        ...
        // Build a transfer tx that also has UTxO outputs — this is the attack vector.
        // The transfer debits sender balance, and the UTxO outputs create tokens
        // from nothing since they aren't paid from any UTxO input.
        let payload = TransferPayload {
            recipient,
            amount: 1_000,
            memo: None,
        ...
        // Must be rejected via apply_block_speculatively (overlay path)
        let (receipts, _overlay) = ledger.apply_block_speculatively(&[tx.clone()]).unwrap();
        assert!(
            matches!(receipts[0].status, TransactionStatus::Failed { .. }),
            "transfer+UTxO mixing must be rejected in overlay path"
        );

        // Also rejected via direct apply_transaction path
        let result = ledger.apply_transaction(&tx);
        assert!(result.is_err(), "transfer+UTxO mixing must be rejected in direct path");
    }
}
