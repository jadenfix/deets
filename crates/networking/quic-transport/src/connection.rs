...
impl QuicConnection {
    ...
    pub async fn send(&self, data: impl Into<Bytes>) -> Result<()> {
        let mut stream = self.inner.open_uni()
            .await
    ...
    pub async fn send_request(&self, data: impl Into<Bytes>) -> Result<Vec<u8>> {
        let (mut send, mut recv) = self.inner.open_bi()
            .await
        ...
        send.finish()
            .await
            .context("Failed to finish send")?;
        
        // Receive response
        let response = recv.read_to_end(10_000_000) // 10MB max response
            .await
            .context("Failed to read response")?;
        
        debug!("Sent {} bytes, received {} bytes from {}", 
               data.len(), response.len(), self.remote());
...
mod tests {
    ...
    async fn test_bidirectional_request() {
        ...
        tokio::time::sleep(tokio::time::Duration::from_millis(10)).await;
        let conn = client.connect(server_addr).await.unwrap();
        let response = conn.send_request(Bytes::from_static(b"ping")).await.unwrap();
