...
impl EntryPoint {
    ...
    pub fn validate_user_op(
        ...
        if let Some(paymaster) = &op.paymaster {
            ...
            let total_gas = op
                .total_gas()
                .ok_or_else(|| anyhow::anyhow!("total_gas overflow"))?;
            let required_gas_cost = (total_gas as u128).saturating_mul(op.max_fee_per_gas);
        ...
        for op in ops {
            let result = match self.validate_user_op(op, validator) {
                Ok(()) => {
                    ...
                    if let Some(paymaster) = &op.paymaster {
                        let total_gas = op.total_gas().unwrap_or(u64::MAX);
                        let cost = (total_gas as u128).saturating_mul(op.max_fee_per_gas);
...
mod tests {
    ...
    /// Accept-all validator for tests that don't focus on signatures.
    struct AcceptAll;
    ...
    /// Reject-all validator for testing invalid signatures.
    struct RejectAll;
    ...
    fn test_entrypoint_rejects_invalid_signature() {
        ...
        assert!(
            result.unwrap_err().to_string().contains("invalid signature"),
    ...
    #[test]
    fn test_total_gas_overflow() {
        let mut op = make_user_op(1);
        op.call_gas_limit = u64::MAX;
        op.verification_gas_limit = 1;
        assert_eq!(op.total_gas(), None);
    }
}
