...
mod tests {
    ...
    /// quorum_percentage > 100 must cause finalize_proposal to return an
    /// error instead of computing a threshold that can never be reached.
    #[test]
    fn test_quorum_percentage_above_100_is_rejected() {
        ...
        // Register a voter so a proposal can be submitted
        let voter = addr(1);
