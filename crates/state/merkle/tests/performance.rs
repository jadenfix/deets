...
fn test_merkle_incremental_updates_performance() {
    ...
    assert!(
        root_compute_time.as_millis() < 50,
        "Root computation took too long: {:?}",
        root_compute_time
    );
}
...
fn test_lazy_computation_efficiency() {
    ...
    assert!(
        first_root_time > cached_time,
        "First access should compute, cached should not"
    );
}
...
fn test_large_tree_performance() {
    ...
    assert!(
        update_time.as_secs() < 1,
        "Updates took too long: {:?}",
        update_time
    );
    assert!(
        root_time.as_millis() < 500,
        "Root computation took too long: {:?}",
        root_time
    );
}
...
fn test_delete_performance() {
    ...
    assert!(
        delete_time.as_micros() < 5_000,
        "Deletes took too long: {:?}",
        delete_time
    );
}
